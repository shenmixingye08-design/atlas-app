...
import { probeXPostBatchSchema } from "@/lib/integrations/x/post/batch-schema-probe";
...
function buildSafeBody(
  result: Awaited<ReturnType<typeof probeXPostBatchSchema>>,
) {
  const version = getHealthVersionPayload();
  return {
    ...toPublicHealthResponse({ ok: result.ok }, { cached: false }),
    batchesTableOk: result.batchesTableOk,
    itemsTableOk: result.itemsTableOk,
...
export async function GET(request: Request): Promise<Response> {
  ...
  const result = await probeXPostBatchSchema({ apply });
  lastRunAtMs = Date.now();
  lastOk = result.ok;
  const body = buildSafeBody(result);
  lastSafeBody = body;

  console.info("[health/x-post-batch]", {
    ok: result.ok,
    batchesTableOk: result.batchesTableOk,
    itemsTableOk: result.itemsTableOk,
