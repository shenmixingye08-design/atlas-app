...
import { probeSideEffectIdempotencySchema } from "@/lib/side-effects/schema-probe";
...
/**
 * P1-04 side-effect idempotency readiness probe.
...
function buildSafeBody(
  result: Awaited<ReturnType<typeof probeSideEffectIdempotencySchema>>,
) {
  const version = getHealthVersionPayload();
  return {
    ...toPublicHealthResponse({ ok: result.ok }, { cached: false }),
    claimsTableOk: result.claimsTableOk,
    claimRpcOk: result.claimRpcOk,
    durableIdempotencyReady: result.durableIdempotencyReady,
...
export async function GET(request: Request): Promise<Response> {
  ...
  const result = await probeSideEffectIdempotencySchema({ apply });
  lastRunAtMs = Date.now();
  lastOk = result.ok;
  const body = buildSafeBody(result);
  lastSafeBody = body;

  console.info("[health/side-effect-idempotency]", {
    ok: result.ok,
    claimsTableOk: result.claimsTableOk,
    claimRpcOk: result.claimRpcOk,
