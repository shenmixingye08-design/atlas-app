...
機能名：Production CORE LOOP E2E（Clerk公式 Playwright helpers + Secret）
...
AIなしで実装可能：はい — Playwright + `@clerk/testing`（clerkSetup / Testing Token / Agent Tasks）+ 既存 API

運営コスト：手動/CI 実行時のみ。AI は検証依頼 1 件分。定期自動実行はしない（workflow_dispatch）

外部APIコスト：有 — Clerk Testing Token + Agent Tasks API（短命）+ 既存 OpenAI（検証依頼時のみ）

コスト削減案：

- [x] エコモードで足りるか — 検証は Word 短文 1 件のみ
- [x] まとめて生成できるか — 対象外（単一ループ証明）
- [x] キャッシュ再利用できるか — トークン / Agent Task は毎回短命発行（再利用しない）
