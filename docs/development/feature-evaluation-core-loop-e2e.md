...
機能名：Production CORE LOOP E2E（Clerk公式 Playwright signIn + Secret）
...
AIなしで実装可能：はい — Playwright + `@clerk/testing`（clerkSetup / Testing Token / clerk.signIn）+ 既存 API

運営コスト：手動/CI 実行時のみ。AI は検証依頼 1 件分。定期自動実行はしない（workflow_dispatch）

外部APIコスト：有 — Clerk Testing Token + Sign-in Token（短命）+ 既存 OpenAI（検証依頼時のみ）
