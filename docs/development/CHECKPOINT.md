...
- **結果画面の文言**: 「あと14.9分の仕事を削減しました」（「あと」＝残りの意味で誤り）→「約14.9分の作業を削減しました」（`lib/i18n/ja.ts` 2箇所）。数値形式はテストで固定のため変更なし。
- 自動化カード下部の余白: 計測の結果、パディング＋44pxのタップ領域で不具合ではない（対応不要）。
- 結果画面（`secretary-result-view`）・通知パネル・成果物一覧は既にキットと整合していることを確認。
- サイドバー「その他」の絵文字（📌・▾）をアイコンセット（新規 `IconMore`・`IconChevron`）に置換。
- ダークモード一括確認（ホーム・今日・設定・自動化・X投稿）: 問題なし。

### 次
1. 実認証の Vercel Preview で通し確認（`/workspace/x` 連携 → 初回完了 → ホーム AIコア完了表示）。
2. PR 作成（ユーザー指示待ち）。
3. 旧画面（`components/home/*` の旧ホーム、`sidebar-items.ts` の旧ナビ）はフラグOFF時のみ。削除は本番フラグ確認後。
