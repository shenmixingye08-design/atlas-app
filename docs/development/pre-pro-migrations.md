...
| 5 | Phase 2: billing durability | `20260713_atlas_billing_subscriptions.sql` | Stripe subscription state と webhook idempotency を `.data` ではなく Supabase に保存 | Production で専用テーブル未作成を確認。ホットパスは `atlas_user_state` domain `atlasBilling` / webhook durable にフォールバック（schema-cache warn を出さない）。専用テーブルは `/api/health/billing-schema?apply=1` または GitHub Action（`DATABASE_URL` / `SUPABASE_ACCESS_TOKEN`+`SUPABASE_PROJECT_REF`）または SQL Editor | `create table if not exists`。既存subscription rowは保持。billing state table追加 | `atlas_billing_subscriptions`, `atlas_stripe_webhook_events` RLS enabled。deny-all | customer id partial index, subscription id partial index, webhook event primary key | なし | `drop policy ...; drop table if exists public.atlas_stripe_webhook_events; drop table if exists public.atlas_billing_subscriptions;`。本番rollback前に subscription/idempotency backup 必須 |
...
| 12 | Phase 5+: deliverable files | `20260726_atlas_deliverable_files.sql` | Word/PDF durable file rows | 未確認 | create if not exists | deny-all | user/expires indexes | Storage は別 migration | drop table |
| 13 | Phase 5+: reliability events | `20260726_atlas_reliability_events.sql` + `20260730_atlas_reliability_events_diagnostics.sql` | SRE / job diagnostics (`job_id`, `stage`, `severity`, …) | 未確認（Production で schema cache miss あり） | create if not exists + alter add column | deny-all | job/stage/created indexes | なし | drop policy; drop table（診断ログ消失） |
| 14 | Phase 5+: deliverable storage | `20260727_atlas_deliverable_files_storage.sql` | Storage bucket / object paths for deliverables | 未確認 | storage policies | — | — | yes | drop policies / objects |
