...
function toStepResult(
  ...
  if (
    ...
    if (!evidenceAllowsCompleted(evidence, Boolean(result.externalUrl))) {
      return {
        ok: false,
        summary: "Evidence不足のため成功扱いできません",
        artifacts: [],
        errorCode: "missing_external_action_id",
        errorMessage: "integration_evidence_incomplete",
        retryable: false,
      };
    }
    ...
    return {
      ...
      errorMessage: null,
      evidence: {
        externalActionIds: result.externalActionId
          ? [result.externalActionId]
          : [],
        externalUrls: result.externalUrl ? [result.externalUrl] : [],
