...
export async function runAutomationLiveAdapterPreflight(input: {
  ...
  for (const step of enabledSteps) {
    ...
    // Destination / recipient gaps are execute-time needs_input — warn only.
    // Blocking here would prevent activate→needs_input→resume flows.
    if (service === "dropbox") {
      const dest =
      ...
      if (!dest) {
        issues.push({
          ...
          message: "Dropbox保存先が未設定です（実行時に入力が必要）",
          blocking: false,
        });
      }
    }
