...
export async function withAdapterGuards(input: {
  ...
  try {
    // Retry only 429 / timeout / network / 5xx — never arbitrary 4xx.
    const outcome = await executeWithRetryPolicy(
      async (attempt) => {
        ...
        if (
          ...
          result.retryable &&
          isRetryable(
            new Error(`${result.errorCode ?? "error"} ${result.summary}`),
          ) &&
          attempt < 3
        ) {
          throw new Error(
            `${result.errorCode ?? "provider_unavailable"}: ${result.summary}`,
          );
        }
        return result;
      },
      { maxAttempts: 3, baseDelayMs: 150, label: input.adapter.service },
    );
    const result = {
      ...outcome.value,
      metadata: {
