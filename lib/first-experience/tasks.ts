...
export const FIRST_EXPERIENCE_TASKS: readonly FirstExperienceTaskDefinition[] = [
  {
    ...
    deliverable: {
      title: "SNS投稿文",
      preview:
        "【今日のひとこと】\n毎日のルーティンをATLASに任せると、投稿・資料・メールまで最後まで進みます。まずは1件、試してみませんか？",
      format: "テキスト",
    },
    employeeSteps: [
  ...
  },
  {
    ...
    deliverable: {
      title: "依頼への回答",
      preview:
        "まずは繰り返しの仕事を1つ選び、ATLASに任せてみましょう。SNS・ブログ・資料のいずれかから始めるのがおすすめです。",
      format: "回答",
    },
    employeeSteps: [
      { icon: "📝", role: "ATLAS", status: "分析中" },
      { icon: "📝", role: "ATLAS", status: "回答作成中" },
      ...BASE_EMPLOYEE_TAIL.slice(1),
    ],
...
export function getFirstExperienceTask(
  ...
  if (taskId === "custom") {
    ...
    return {
      ...
      deliverable: {
        title: "成果物",
        preview: `${text}について、ATLASが対応方針をまとめました。ホームから本格的な仕事を続けられます。`,
        format: "テキスト",
      },
      employeeSteps: [
        { icon: "🤖", role: "ATLAS", status: "分析中" },
        { icon: "🤖", role: "ATLAS", status: "作成中" },
        ...BASE_EMPLOYEE_TAIL.slice(1),
      ],
