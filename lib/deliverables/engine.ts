...
export async function rerenderDeliverables(
  ...
  for (const format of options.formats) {
    ...
    if (!options.userId) {
      throw new Error("userId is required to rerender deliverables");
    }
    const storedFile = saveDeliverableFile(file, options.userId, {
      deliverableId: artifact.id,
      sourceContent: documentModelToExportText(model),
