...
function addIconBadge(slide: pptxgen.Slide, label: string, x = 0.6, y = 1.25): void {
  ...
  for (const section of parsed.sections) {
    ...
    if (body.trim()) {
      chunks.forEach((chunk, index) => {
        ...
        addIconBadge(slide, String(index + 1));
    ...
    for (const table of tableBlocks) {
      ...
      const slide = pptx.addSlide();
      addTableSlide(slide, section.title, table.headers, table.rows);
      slides.push(slide);
    }
