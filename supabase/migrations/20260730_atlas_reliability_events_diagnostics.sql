-- Job / stage diagnostics for atlas_reliability_events.
-- Safe to re-run. Service-role writes only (RLS deny-all for anon/auth).
...
-- Backfill columns when an older metrics-only table already exists.
alter table public.atlas_reliability_events
...
update public.atlas_reliability_events
set metadata = '{}'::jsonb
...
-- Keep message / error_message in sync for older readers.
update public.atlas_reliability_events
...
create index if not exists atlas_reliability_events_diag_created_idx
  on public.atlas_reliability_events (diagnostic_id, created_at desc);

create index if not exists atlas_reliability_events_stage_created_idx
  on public.atlas_reliability_events (stage, created_at desc);
