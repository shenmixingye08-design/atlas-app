...
import { LegalItemCard } from "@/components/legal/legal-item-card";
import { LegalTableOfContents } from "@/components/legal/legal-table-of-contents";
import { LEGAL_ITEMS, LEGAL_META } from "@/lib/legal";
import type { LegalArticle } from "@/lib/legal/types";
import { ui } from "@/lib/i18n";

function sectionLabel(article: LegalArticle): string {
  return article.sectionPrefix ?? `${article.number}`;
}
        ...
        <div className="mx-auto max-w-[800px] lg:max-w-none">
          <div className="mb-10 space-y-3 lg:mb-12">
            <p className="text-sm font-medium text-[var(--terms-accent)]">
              {ui.legal.commercialBadge}
            </p>
            <h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">
              {ui.legal.commercialTitle}
            </h1>
            <p className="text-base leading-relaxed text-[var(--terms-muted)]">
              {ui.legal.commercialIntro}
            </p>
            <p className="text-sm text-[var(--terms-muted)]">
              {ui.legal.effectiveDate(LEGAL_META.effectiveDateDisplay)}
            </p>
          </div>
          ...
          <div className="terms-layout grid gap-10 lg:grid-cols-[220px_minmax(0,800px)] lg:gap-14 xl:grid-cols-[240px_minmax(0,800px)]">
            <aside className="terms-toc-aside lg:sticky lg:top-24 lg:self-start">
              ...
              <LegalTableOfContents
                articles={LEGAL_ITEMS}
                className="hidden lg:block"
                formatSectionLabel={sectionLabel}
              />
            </aside>

            <main className="terms-main min-w-0 space-y-4">
              {LEGAL_ITEMS.map((item) => (
                <LegalItemCard
                  key={item.id}
                  item={item}
              ...
              <footer className="terms-meta border-t border-[var(--border-subtle)] pt-8">
                <dl className="grid gap-3 text-sm text-[var(--terms-muted)] sm:grid-cols-2">
                  <div>
                    ...
                    <dd>{LEGAL_META.lastUpdatedDisplay}</dd>
                  </div>
                  <div>
                    <dt className="font-medium text-[var(--terms-heading)]">
