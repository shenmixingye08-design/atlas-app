 ...
 * Speculation forbidden: all numbers come from this run.
 */
...
const RUNS = Number(process.argv[2] ?? process.env.RELIABILITY_RUNS ?? 1000);
const OUT_DIR =
  process.argv[3] ??
...
async function oneIteration(i: number): Promise<IterResult> {
  ...
  // Simulate multi-instance: clear memory, reload from durable/disk.
  resetDeliverableMemoryStoreForTests();
...
async function main() {
  ...
  console.log(`[reliability-e2e] starting ${RUNS} runs → ${OUT_DIR}`);
  ...
  for (let i = 1; i <= RUNS; i += 1) {
    ...
    if (i % 50 === 0 || i === RUNS) {
      ...
      console.log(
        `[reliability-e2e] ${i}/${RUNS} ok=${okCount} fail=${i - okCount}`,
      );
    }
  }
  ...
  writeFileSync(
    ...
    `# Reliability 1000-run Report (measured)
...
${
  failures.length === 0
    ...
    : failures
        .slice(0, 20)
        .map((f) => `- #${f.i}: ${f.reasons.join("; ")}`)
        .join("\n")
}
`,
  );

  console.log(JSON.stringify({ gatePass, successCount, failCount, avg, p95: report.p95Ms, p99: report.p99Ms, p999: report.p999Ms }, null, 2));
